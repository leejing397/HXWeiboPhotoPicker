...
- (void)cellDidSelectedBtnClick:(HXPhotoViewCell *)cell Model:(HXPhotoModel *)model {
    //判断image的创建时间是否在今天的前后3内，若不是不能选择此照片
...
- (void)cleanSelectedList {
    ...
    if (!self.manager.singleSelected) {
        ...
        self.manager.endSelectedPhotos = self.manager.selectedPhotos;
        ...
        if ([self.delegate respondsToSelector:@selector(photoViewControllerDidNext:Photos:Videos:Original:)]) {
            [self.delegate photoViewControllerDidNext:self.manager.endSelectedList.mutableCopy Photos:self.manager.endSelectedPhotos Videos:self.manager.endSelectedVideos.mutableCopy Original:self.manager.endIsOriginal];
        }
    }else {
        if ([self.delegate respondsToSelector:@selector(photoViewControllerDidNext:Photos:Videos:Original:)]) {
